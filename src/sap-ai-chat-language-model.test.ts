...
describe("SAPAIChatLanguageModel", () => {
  ...
  describe("request format", () => {
    it("should include parallel_tool_calls when parallelToolCalls is set", async () => {
      const mockResponse = {
        ok: true,
        status: 200,
        statusText: "OK",
      ...
      // Recreate model with parallelToolCalls enabled=false
      model = new SAPAIChatLanguageModel(
        "gpt-4o",
        { modelParams: { parallelToolCalls: false } },
        {
          provider: "sap-ai",
          baseURL:
            "https://api.ai.test.com/v2/inference/deployments/test-deployment/completion",
